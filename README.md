...
- 教學內容：
  ...
  - 類別型預處理：類別型遺漏值處理、OneHotEncoder
  - Pipeline 管道器串接多個轉換器
  - ColumnTransformer 水平合併器、KBinsDiscretizer
...
- 教學內容：
  ...
  - 資料切割：train_test_split
  - LinearRegression 模型訓練與評估
  - PolynomialFeatures 多次方轉換器
  - 多元迴歸、迴歸係數檢視、特徵值選擇、對數轉換

### 第 5 週｜羅吉斯迴歸與分類評估

- 對應教科書：Ch6 羅吉斯迴歸
- 教學內容：
  - Sigmoid 函數原理、判斷邊界的概念
  - 羅吉斯迴歸模型訓練與參數取得
  - 混淆矩陣、Classification Report
  - 預測機率與門檻調整
  - Precision-Recall Curve、ROC 曲線與 AUC
  - **組內討論投票決定題目、繳交專題提案**
...
- 教學內容：
  - KNN 模型建構、標準化對 KNN 的影響、K 值選擇
  - PCA 降維、SelectKBest 特徵選擇
...
- 教學內容：
  - 決策樹建構、過擬合控制：max_depth、min_samples_split
  - 特徵重要性分析、決策樹視覺化
  - 分類預測模板：自動取出欄位、多模型結果比較
...
- 教學內容：
  ...
  - GridSearchCV 實作、最佳參數取得

### 第 9 週｜期中考

- 考試範圍：第 1 至 8 週，涵蓋 Ch0 ~ Ch12
- 考試形式：筆試 + Colab 實作
...
- 對應教科書：Ch14 員工流失率預測
- 教學內容：
  - 完整建模流程：資料探索、管道器建構、GridSearchCV、ROC 曲線
  - AI 輔助資料清洗與模型選擇
...
- 對應教科書：Ch17 文字處理
- 教學內容：
  - 詞袋模型 Bag of Words、n-gram、停用詞移除
  - TF-IDF 加權
  - LDA 主題探索、預測文章所屬主題
...
- 教學內容：
  - Amazon 評論情感分類、Naive Bayes、不均衡處理
  - jieba 中文斷詞、中文詞袋處理、LDA 中文主題分析、文字雲

### 第 15 週｜K-Means 集群分析與綜合實作

- 對應教科書：Ch20 K-means 集群分析
- 教學內容：
  - K-Means 集群分析實作、Inertia、手肘法
  - 集群結果推論與視覺化
  - AI 輔助完整資料探勘分析流程
...
- 教學內容：
  - 手寫數字辨識、Keras 架構建構
  - KerasClassifier 整合 scikit-learn
  - 傳統機器學習 vs 深度學習效能比較
